...
if (isset($_SESSION['user_id']) and (strpos($_SESSION['user_permissions'],"list")!==false))
{
	...
	while ($row=mysql_fetch_array($res)) {
		...
		$pos = world_pos($Worldspace, str_replace("dayz_", "", $database_name));
		/*if(world_pos_x_gps($Worldspace, str_replace("dayz_", "", $database_name)) > 100)
		{
			$pos = "<font color=#00FF00>".world_pos($Worldspace, str_replace("dayz_", "", $database_name))." (Greenzone)</font>";
		}
		else
		{
			$pos = "<font color=#FF0000>".world_pos($Worldspace, str_replace("dayz_", "", $database_name))." (Banditland)</font>";
		}*/
