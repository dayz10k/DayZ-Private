...
foreach my $script (@scripts) {
	...
	my $cmd = (($^O =~ m/MSWin32/) ? 'wget\wget' : 'wget');
	$cmd = "$cmd --no-check-certificate -q -N -O $dst/$script $uri";

	print "Fetching URI $uri\n";
	my $ret = system($cmd);

	die "> Error: Could not fetch URI!" unless ($ret == 0);
...
print "Update complete. NOTE: You must reload the filters on a running server for changes to take effect!\n";
