									...
									if ($file != "") {
										...
										if (file_exists($filepath) && is_file($filepath)) {
											if (isset($_GET['confirm'])) {
												...
												echo '<script type="text/javascript">if (confirm("Do you really want to delete \"'.$_GET['delete'].'\"?")) { window.location = "index.php?view=log&type='.$_GET['type'].'&delete='.$_GET['delete'].'&confirm"; }</script>';
											}
										} else {
											echo '<div id="page-heading"><h2>File not found: '.$filepath.'<h2></div>';
										}
