<?
	$query = "SELECT deployable.class_name, instance_deployable.* FROM `deployable`, `instance_deployable` AS `instance_deployable` WHERE deployable.id = instance_deployable.deployable_id AND unique_id = '".$_GET["id"]."' LIMIT 1"; 
	...
	while ($row=mysql_fetch_array($res)) {
		...
		$owneruid = "";
		if ($row['owner_id'] != "0"){
			$query2 = "SELECT profile.name, survivor.* FROM `profile`, `survivor` AS `survivor` WHERE profile.unique_id = survivor.unique_id AND survivor.id = '".$row['owner_id']."' LIMIT 1"; 
