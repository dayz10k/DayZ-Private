<?php
	$res = mysql_query("SELECT profile.name, survivor.* FROM `profile`, `survivor` AS `survivor` WHERE profile.unique_id = survivor.unique_id AND survivor.unique_id = '".$_GET["id"]."' AND survivor.id = '".$_GET["cid"]."' LIMIT 1") or die(mysql_error());
					...
					<div id="gear_player">	
						<div class="gear_info">
							...
							<div class="statstext" style="width:180px;margin-left:205px;margin-top:-55px">
								<?php echo 'Survivor killed:&nbsp;'.$row['survivor_kills']; ?>
							</div>
							<div class="statstext" style="width:180px;margin-left:205px;margin-top:-35px">
								<?php echo 'Bandit killed:&nbsp;'.$row['bandit_kills']; ?>
							</div>
						</div>
